...
with tabs[0]:
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=hours, y=gen_24, name="Solar Gen", fill='tozeroy', line=dict(color='#64ffda', width=4)))
    fig.add_trace(go.Scatter(x=hours, y=load_24, name="Load", line=dict(color='#8892b0', width=3)))
    if has_batt: fig.add_trace(go.Scatter(x=hours, y=soc, name="Battery", line=dict(color='#22c55e', width=3)))
    fig.update_layout(height=500, plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font_color='#e6f1ff')
